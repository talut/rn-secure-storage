                  ...
                  rejecter:(RCTPromiseRejectBlock)reject)
{
    @try {
        ...
        if (status) {
            resolve(@"{\"message\":\"Key stored successfully\", \"status\":"+true+"}");
        } else {
            BOOL status = [self updateKeychainValue: value forIdentifier: key options: options];
            if (status) {
                resolve(@"{\"message\":\"Key updated successfully\", \"status\":"+true+"}");
            } else {
                NSString* errorMessage = @"{\"message\":\"An error occurred\", \"status\":"+false+"}";
                reject(@"9", errorMessage, secureKeyStoreError(errorMessage));
            }
        }
    }
    @catch (NSException *exception) {
        NSString* errorMessage = [NSString stringWithFormat:@"{\"message\":\"An error occurred\", \"status\":"+false+"}", exception];
        reject(@"9", errorMessage, secureKeyStoreError(errorMessage));
    }
}
                  ...
                  rejecter:(RCTPromiseRejectBlock)reject)
{
    @try {
        ...
        if (value == nil) {
            NSString* errorMessage = @"{\"message\":\"key does not present\", \"status\":"+false+"}";
    ...
    @catch (NSException *exception) {
        NSString* errorMessage = [NSString stringWithFormat: @"{\"message\":\"key does not present\", \"status\":"+false+"}", exception];
                  ...
                  rejecter:(RCTPromiseRejectBlock)reject)
{
    @try {
        ...
        if (status) {
            resolve(@"{\"message\":\"key removed successfully\", \"status\":"+true+"}");
        } else {
            NSString* errorMessage = @"{\"message\":\"An error occurred when key removed.\", \"status\":"+false+"}";
            reject(@"6", errorMessage, secureKeyStoreError(errorMessage));
        }
    }
    @catch(NSException *exception) {
        NSString* errorMessage = [NSString stringWithFormat:@"{\"message\":\"An error occurred when key removed.\", \"status\":"+false+"}", exception];
        reject(@"6", errorMessage, secureKeyStoreError(errorMessage));
    }
}
